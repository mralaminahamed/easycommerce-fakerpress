...
import type { ParameterConfig, GeneratorBaseProps } from "@/admin/types";
        ...
        return (
          <div className="flex items-center relative">
            <Switch
              ...
              disabled={isLoading}
            />
            <Label className="ml-3 text-sm font-medium text-wp-text">
              {config.title ||
                config.description ||
                getFieldLabel(paramName, config)}
            </Label>
        ...
        if (config.items && config.items.enum) {
          ...
          return (
            <div className="space-y-2 relative">
              <div className="flex items-center space-x-2">
                <Label className="block text-sm font-medium text-wp-text">
                  {config.title ||
                    config.description ||
                    getFieldLabel(paramName, config)}
                </Label>
            ...
            <CardContent>
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                <div className="space-y-1 relative">
                  <Label
                    htmlFor={`${type}-count`}
                    className="block text-sm font-medium text-wp-text"
                  >
                    {
                  ...
                  <Input
                    ...
                    className="w-32"
                    disabled={isLoading}
                    aria-describedby={`${type}-count-description`}
                    onChange={(e: React.ChangeEvent<HTMLInputElement>) =>
                      setCount(parseInt(e.target.value))
                    }
                  />
                  <div id={`${type}-count-description`} className="sr-only">
                    {__(
