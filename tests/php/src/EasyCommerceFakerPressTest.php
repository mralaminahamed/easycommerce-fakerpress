...
class EasyCommerceFakerPressTest extends EasyCommerceFakerPressUnitTestCase {
	...
	public function test_add_admin_menu(): void {
		...
		// Clear existing menu.
		$admin_page_hooks = array(); // phpcs:ignore WordPress.WP.GlobalVariablesOverride.Prohibited
		...
		$this->assertArrayHasKey( 'easycommerce-fakerpress', $GLOBALS['admin_page_hooks'] );
	}
	...
	public function test_render_admin_page(): void {
		...
		$this->assertStringContainsString( '<div id="easycommerce-fakerpress-root"></div>', $output );
	}
	...
	public function test_handle_ajax_request_valid_params(): void {
		...
		$mock_generator->method( 'generate' )->willReturn(
			array(
				'success'          => true,
				'products_created' => 5,
			)
		...
		add_filter(
			...
			function ( $plugins ) {
				...
				return array_filter(
					$plugins,
					function ( $plugin ) {
						return 'easycommerce/easycommerce.php' !== $plugin;
					}
				);
			}
		);
	...
	public function test_dependency_notice(): void {
		...
		$this->assertStringContainsString( 'EasyCommerce FakerPress requires EasyCommerce plugin', $output );
	}
}
