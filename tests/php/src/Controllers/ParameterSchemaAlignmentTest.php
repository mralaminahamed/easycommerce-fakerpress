...
class ParameterSchemaAlignmentTest extends EasyCommerceFakerPressUnitTestCase {
	...
	public function test_products_generator_parameter_alignment(): void {
		...
		$request->set_param( 'content_options', [
	...
	public function test_orders_generator_parameter_alignment(): void {
		...
		$request->set_param( 'customer_distribution', [
			'existing_ratio' => 70,
			'new_ratio' => 30
		]);

		// Test aligned order_value parameter
		$request->set_param( 'order_value', [
			'min_total' => 25,
			'max_total' => 200
		]);

		// Test aligned items_per_order parameter
		$request->set_param( 'items_per_order', [
			'min' => 1,
			'max' => 5
		]);

		// Test aligned payment_methods parameter
		$request->set_param( 'payment_methods', ['stripe', 'paypal'] );

		// Test aligned geographical_distribution parameter
		$request->set_param( 'geographical_distribution', [
			'countries' => ['US', 'CA']
		]);
	...
	public function test_customers_generator_parameter_alignment(): void {
		...
		$request->set_param( 'customer_types', ['regular', 'vip'] );

		// Test aligned demographics parameter
		$request->set_param( 'demographics', [
			'age_groups' => ['26-35', '36-45'],
			'gender_distribution' => [
				'male' => 45,
				'female' => 45,
				'other' => 10
		...
		$request->set_param( 'address_preferences', [
		...
		$request->set_param( 'purchase_history', [
			'simulate_history' => true,
			'loyalty_tiers' => true
		]);

		// Test aligned contact_preferences parameter
		$request->set_param( 'contact_preferences', [
			'phone_numbers' => true,
	...
	public function test_coupons_generator_parameter_alignment(): void {
		...
		$request->set_param( 'discount_types', ['percentage', 'fixed_amount'] );

		// Test aligned discount_range parameter
		$request->set_param( 'discount_range', [
			'min_percentage' => 10,
			'max_percentage' => 30,
			'min_fixed' => 5,
			'max_fixed' => 50
		...
		$request->set_param( 'usage_limits', [
		...
		$request->set_param( 'validity_period', [
			'min_days' => 7,
			'max_days' => 60
		]);

		// Test aligned restrictions parameter
		$request->set_param( 'restrictions', [
			'minimum_spend' => true,
			'maximum_spend' => false,
	...
	public function test_cart_sessions_generator_parameter_alignment(): void {
		...
		$request->set_param( 'status_distribution', [
			'pending' => 20,
			'abandoned' => 60,
			'completed' => 20
		]);
		...
		$request->set_param( 'cart_value_range', [
			'min' => 10,
			'max' => 150
		]);

		// Test aligned items_per_cart parameter
		$request->set_param( 'items_per_cart', [
			'min' => 1,
			'max' => 4
		]);

		// Test aligned abandonment_tracking parameter
		$request->set_param( 'abandonment_tracking', [
	...
	public function test_transactions_generator_parameter_alignment(): void {
		...
		$request->set_param( 'order_status_filter', ['completed', 'processing'] );

		// Test aligned transaction_types parameter
		$request->set_param( 'transaction_types', ['payment', 'refund'] );

		// Test aligned payment_gateways parameter
		$request->set_param( 'payment_gateways', ['stripe', 'paypal'] );

		// Test aligned date_range parameter
		$request->set_param( 'date_range', [
			'start' => '2024-01-01',
			'end' => '2024-12-31'
		]);
	...
	public function test_locations_generator_parameter_alignment(): void {
		...
		$request->set_param( 'regions', ['Europe', 'Americas'] );
		...
		$request->set_param( 'cities_per_state', [
			'min' => 2,
			'max' => 10
		]);
	...
	public function test_shipping_plans_generator_parameter_alignment(): void {
		...
		$request->set_param( 'shipping_types', ['standard', 'express', 'free'] );

		// Test aligned cost_range parameter
		$request->set_param( 'cost_range', [
			'min' => 0,
			'max' => 25
		]);

		// Test aligned coverage_areas parameter
		$request->set_param( 'coverage_areas', ['domestic', 'international'] );

		// Test aligned calculation_methods parameter
		$request->set_param( 'calculation_methods', ['flat_rate', 'weight_based'] );

		// Test aligned delivery_timeframes parameter
		$request->set_param( 'delivery_timeframes', [
			'min_days' => 1,
			'max_days' => 7
		]);
	...
	public function test_tax_classes_generator_parameter_alignment(): void {
		...
		$request->set_param( 'tax_types', ['standard', 'reduced', 'zero'] );

		// Test aligned jurisdictions parameter
		$request->set_param( 'jurisdictions', ['country', 'state'] );

		// Test aligned rate_ranges parameter
		$request->set_param( 'rate_ranges', [
			'standard' => [
				'min' => 5,
				'max' => 20
			],
		...
		$request->set_param( 'location_coverage', [
			'countries' => ['US', 'CA', 'GB'],
	...
	public function test_product_variations_generator_parameter_alignment(): void {
		...
		$request->set_param( 'product_types', ['variable', 'simple'] );

		// Test aligned price_variance parameter
		$request->set_param( 'price_variance', [
			'min_percentage' => -15,
			'max_percentage' => 25
		]);

		// Test aligned stock_settings parameter
		$request->set_param( 'stock_settings', [
			'manage_stock' => true,
			'stock_range' => [
				'min' => 0,
				'max' => 30
		...
		$request->set_param( 'variation_attributes', [
